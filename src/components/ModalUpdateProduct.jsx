...
export const ModalUpdateProduct = ({setIsOpenModal, productSelected}) => {
  ...
  const [checked, setChecked] = useState(productSelected &&  productSelected.inSale !== '' ? true : false)
        ...
        <div className="flex gap-2">
          <label className="flex flex-col space-y-2 w-full">
            ...
            <input
              ...
              required
              name="expiresIn"
              defaultValue={new Date(productSelected.expiresIn).toISOString().split("T")[0]}
              className="p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </label>
          <label className="flex flex-col space-y-2 w-full">
